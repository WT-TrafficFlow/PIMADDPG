...
class MAMGCN_encblock(nn.Module):
    ...
    def forward(self, x, TE, SE, adj):
        ...
        trend_init = trend_init.transpose(2, 3)
        # TAt
        ...
        spatial_gate = self.SAt(x_TAt)
        trend_enc = self.cheb_conv_SAt(trend_init, spatial_gate) # b n f l
        ...
        seasonal_init = seasonal_init + TE + SE
        ...
        x_residual = self.ln(x + seasonal_enc + trend_enc)
...
class MAMGCN_decblock(nn.Module):
    ...
    def __init__(self, DEVICE, in_channels, K, nb_chev_filter, num_heads, dropout, cheb_polynomials, num_of_vertices, num_of_timesteps):
        ...
        # 空间-交叉
        self.SDecoderLayer = nn.TransformerDecoderLayer(
            d_model=self.enc_dim,
            nhead=self.num_heads,
            dim_feedforward=self.enc_dim * 4, batch_first=True)
        self.csat = nn.TransformerDecoder(self.SDecoderLayer, num_layers=1)
