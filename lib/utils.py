...
def generate_data(dataset_dir):
    ...
    for category in ['train', 'val', 'test']:
        ...
        data['y_' + category] = np.transpose(cat_data['y'], (0, 2, 1, 3))
        # x = cat_data['x'][..., :1]
        # y = cat_data['y'][..., :1]
        # if mean is None:
...
def compute_val_loss_mstgcn(net, val_loader, sw, epoch, limit=None):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(val_loader):
            ...
            outputs, fd_q = net(encoder_inputs)
            loss_q = masked_mae_loss(outputs[..., 0], labels[..., 0])
            loss_occ = masked_mae_loss(outputs[..., 1], labels[..., 1])
            # if epoch > 5:
            loss_fd = masked_mae_loss(outputs[..., 0], fd_q[..., 0])
            # loss = loss_occ + loss_q / (loss_q / loss_occ).detach() + loss_fd / (loss_fd / loss_occ).detach()
            loss = 0.7*loss_occ + 0.15*(loss_q + loss_fd)

            # else:
            #     loss_fd = masked_mae_loss(fd_q[..., 0], labels[..., 0])
            #     loss = loss_q + loss_occ / (loss_occ / loss_q).detach() + loss_fd / (loss_fd / loss_q).detach()
...
def predict_and_save_results_mstgcn(net, data_loader, data_target_tensor, global_step, _mean, _std, params_path, type):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(data_loader):
            ...
            input.append(encoder_inputs[:, :, :12, :2].cpu().numpy())  # (batch, T', 1)

            outputs, fd_q = net(encoder_inputs)

            prediction.append(outputs.detach().cpu().numpy())
            fd_qs.append(fd_q.detach().cpu().numpy())
            if batch_index % 100 == 0:
        ...
        prediction = np.concatenate(prediction, 0)  # (batch, T', 1)
        fd_qs = np.concatenate(fd_qs, 0)
        print('input:', input.shape)
        print('prediction:', prediction.shape)
        print('fd_qs:', fd_qs.shape)
        print('data_target_tensor:', data_target_tensor.shape)
        output_filename = os.path.join(params_path, 'output_epoch_%s_%s' % (global_step, type))
        # np.savez(output_filename, input=input, prediction=prediction, data_target_tensor=data_target_tensor)

        np.savez_compressed('data/MAMBAGCN-' + 'SUMO_SIM', **{'prediction': prediction, 'truth': data_target_tensor, 'fd_q': fd_qs})
        ...
        for (l,r) in [(0, 12)]:
            ...
            mae, rmse, mape = metric(prediction[:, l:r, :, 0], data_target_tensor[:, l:r, :, 0])
            mae_occ, rmse_occ, mape_occ = metric(prediction[:, l:r, :, 1], data_target_tensor[:, l:r, :, 1])
            mae_fd, rmse_fd, mape_fd = metric(prediction[:, l:r, :, 0], fd_qs[:, l:r, :, 0])
            ...
            print('q_average:         %.3f\t\t%.3f\t\t%.3f%%' %(mae, rmse, mape * 100))
            # excel_list.extend([mae, rmse, mape])
            # print(excel_list)
            print('\n')
            print('occ_average:         %.3f\t\t%.3f\t\t%.3f%%' % (mae_occ, rmse_occ, mape_occ * 100))
            print('\n')
            print('fd_average:         %.3f\t\t%.3f\t\t%.3f%%' % (mae_fd, rmse_fd, mape_fd * 100))
            print('\n')
